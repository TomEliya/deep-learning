...
class DetectFakeReviews:
    ...
    def __train_model(self):
        ...
        self.__ann.add(tf.keras.layers.Dense(units=10, activation="relu"))
        # Adding the second hidden layer
        self.__ann.add(tf.keras.layers.Dense(units=10, activation="relu"))
        # Adding the output layer
        self.__ann.add(tf.keras.layers.Dense(units=1, activation="sigmoid"))
        # Compiling the ANN
        self.__ann.compile(optimizer="adam", loss="binary_crossentropy", metrics=["accuracy"])
        # Training the ANN on the Training set
        self.__ann.fit(X_train, y_train, batch_size=32, epochs=100)

        # Predicting the Test set results
        y_pred = self.__ann.predict(X_test)
        y_pred = y_pred > 0.5
        ...
        return self.__ann, self.__cv, self.__sc
    ...
    def predict_value(self, to_predict: str) -> str:
        ...
        post_prediction = self.__ann.predict(our_real_review)
        post_prediction = post_prediction > 0.5
        if self.__real != None:
